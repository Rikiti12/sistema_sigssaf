                                    ...
                                    <td>
                                        <div style="display: flex; justify-content: center;">
                                            ...
                                            @can('crear-planificacion')
                                                @if(!$asignacion->yaPlanificada)
                                                    <a class="btn btn-primary btn-sm" title="Planificar Proyecto" href="{{ route('planificacion.create', ['id' => $asignacion->id]) }}">
    ...
    <script>
        $(document).ready(function() {
            $('#dataTable').on('click', '.btn-info', function(event) {
                ...
                $.ajax({
                    url: '/planificacion/detalles/' + asignacionId,
                    type: 'GET',
                    success: function(data) {
                        ...
                        if (data.imagenes && data.imagenes.length > 0) {
                            asignacionesHtml += `<p><b>Fotos:</b></p><div style="display: flex; flex-wrap: wrap; gap: 10px;">`;
                            let imagenes = JSON.parse(data.imagenes);
                            imagenes.forEach(function(imagen) {
                                asignacionesHtml += `<img src="/imagenes/${imagen}" width="60%" style="width: calc(50% - 10px); margin-bottom: 10px;">`;
                            });
