                ...
                <div class="card mb-4">
                    ...
                    <form method="post" action="{{ url('/comunidad/'.$comunidad->id) }}" enctype="multipart/form-data" onsubmit="return Comunidad(this)">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <label class="font-weight-bold text-dark">¿Desea editar el proyecto de esta comunidad?</label>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="editar_proyecto" id="si_proyecto" value="si" onchange="mostrarOcultarProyecto()" {{ !empty($comunidad->nom_proyecto) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="si_proyecto">Sí</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="editar_proyecto" id="no_proyecto" value="no" onchange="mostrarOcultarProyecto()" {{ empty($comunidad->nom_proyecto) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="no_proyecto">No</label>
                                    </div>
                                </div>
                            </div>

                            <div id="datos_proyecto" style="display: {{ !empty($comunidad->nom_proyecto) ? 'block' : 'none' }};">
                                <br>
                                <center>
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        ...
                                        <input type="text" class="form-control" id="nom_proyecto" name="nom_proyecto" style="background: white;" value="{{ $comunidad->nom_proyecto ?? '' }}" autocomplete="off" oninput="capitalizarInput('nom_proyecto')" onkeypress="return soloLetras(event);">
                                    </div>

                                    <div class="col-6">
                                        <label  class="font-weight-bold text-dark">Descripción del Proyecto</label>
                                        <textarea class="form-control" id="descripcion" name="descripcion" cols="10" rows="5" style="max-height: 6rem;" oninput="capitalizarInput('descripcion')">{{ $comunidad->descripcion ?? '' }}</textarea>
                                    </div>
                                </div>
                            </div>
